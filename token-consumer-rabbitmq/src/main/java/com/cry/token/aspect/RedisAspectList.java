...
import com.cry.token.redis.RedisCacheList;
import com.cry.token.utils.ClassNameUtils;
...
@Component
@Aspect
public class RedisAspectList {

    //给类初始化日志对象
    private static final Logger LOGGER = LoggerFactory.getLogger(RedisAspectList.class);

    @Autowired
    @Qualifier("redisCacheList")
    private RedisCacheList redisCacheList;

    /**
     * 拦截所有元注解RedisCacheList注解的方法
     */
    @Pointcut("@annotation(com.cry.token.annotation.RedisCacheList)||@within(com.cry.token.annotation.RedisCacheList)")
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws ClassNotFoundException {
        ...
        List<Object> obj = redisCacheList.getListFromRedis(redisKey, ClassNameUtils.reflectClassName(className));
        ...
        String code = redisCacheList.saveListToRedis(redisKey, list);
        if (code.equals("OK")) {
