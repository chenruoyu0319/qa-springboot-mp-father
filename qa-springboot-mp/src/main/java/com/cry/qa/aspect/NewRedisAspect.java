...
import com.cry.qa.annotation.Redis;
import com.cry.qa.annotation.RedisEvict;
import com.cry.qa.constant.RedisReturnType;
import com.cry.qa.redis.RedisCache;
import com.cry.qa.utils.JsonUtil;
import com.cry.qa.utils.RedisUtil;
...
@Component
@Slf4j
public class NewRedisAspect {

    /**
     * 获取或添加缓存
     */
    @Around("@annotation(com.cry.qa.annotation.Redis)")
    public Object Redis(final ProceedingJoinPoint jp) throws Throwable {
        ...
        if (StringUtils.isNotEmpty(redisRS)) {
            ...
            //返回maplist
            if (type.equals(RedisReturnType.MAPLIST.getType())) {
     ...
     * 删除缓存
     */
/*	@Around("@annotation(com.cry.qa.annotation.RedisEvict)")
